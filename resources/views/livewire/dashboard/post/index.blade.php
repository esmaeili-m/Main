<div>
    ...
    @section('breadcrumb')
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">Posts</h1>
            ...
            <div class="card custom-card">
                <div class="card-header justify-content-between">
                    ...
                    <div class="prism-toggle">
                        <button wire:click="resetData()" data-bs-effect="effect-flip-vertical" data-bs-toggle="modal" href="#modaldemo8" class="btn btn-sm btn-success-light">Create New Post<i class="ri-plus ms-2 d-inline-block align-middle"></i></button>
                        ...
                        <table class="table text-nowrap">
                            ...
                            @foreach($data ?? [] as  $item)
                                <tr>
                                    ...
                                    <td>
                                        <img class="posts-image" src="{{asset('storage/'.$item->thumbnail)}}" alt="{{$item->title}}">
                                    </td>
                                    <td>{{$item->title}}</td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="hstack gap-2 flex-wrap">
                                            ...
                                            <a  wire:click="remove_item_post({{$item->id}})"  href="javascript:void(0);" class="text-danger fs-14 lh-1"><i
        ...
        <div class="modal-dialog modal-dialog-centered text-center  modal-xl" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">{{$this->editMode ? 'Update Post' : 'Create Post'}}</h6><button aria-label="Close" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body text-start">
                    <form>
                        <div class="row">
                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-3">
                                <p class="mb-2 text-muted">Post Title</p>
                            ...
                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-3">
                                <p class="mb-2 text-muted">Post Slug</p>
                            ...
                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-3">
                                <p class="mb-2 text-muted">Post Video Url</p>
                                <input id="video_url" wire:model.lazy="video_url" type="text" class="form-control" placeholder="Enter video_url">
                                @error('video_url')
                                <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-3">
                                ...
                                <input wire:model.lazy="thumbnail" class="form-control" type="file" id="input-file">
                                @error('thumbnail')
                                <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-3">
                                <div wire:ignore>
                                    <label for="input-file" class="form-label">Category</label>
                            ...
                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-3">
                                <div>
                                    <label for="input-file" class="form-label">Tags</label>
                                    <select onchange="@this.set('tag',this.value)"  class="form-control"  name="choices-single-default" id="select_tag">
                                        <option  value="">Select Tag</option>
                                        @foreach(\App\Models\Tag::pluck('name','id') ?? [] as $key => $item)
                                            <option value="{{$key}}">{{$item}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="d-flex gap-1 mt-2">
                                    @foreach($tags_labels ?? [] as $tagitem_key => $tagitem)
                                        <button type="button" class="btn btn-danger label-btn label-end rounded-pill">
                                            {{$tagitem}}
                                            <i wire:click="remove_item({{$tagitem_key}})" class="ri-close-line label-btn-icon ms-2 rounded-pill"></i>
                                        </button>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="col-xl-12 col-lg-6 col-md-6 col-sm-12 mb-3">
                                <div wire:ignore>
                                    <label for="input-file" class="form-label">Content Post</label>
        ...
        <script>
            document.addEventListener("livewire:initialized", () => {
                ...
                function setSelectValue() {
                    const value = String(@this.get('category_id') || '').trim();
                ...
                @this.set('category_id', selectEl.value);
                });

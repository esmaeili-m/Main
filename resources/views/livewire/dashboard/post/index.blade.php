            ...
            <div class="card">
                <div class="d-flex justify-content-between align-items-center p-4">
                    ...
                    <div class="btns-table d-flex gap-2">
                        <button wire:click="resetData()" data-toggle="modal" data-target="#create" type="button" class="btn-hover btn-sm btn-border-radius color-5">ایجاد پست</button>
                    ...
                    <table class="table table-striped">
                        ...
                        @foreach($data ?? [] as $item)
                            <tr>
                                ...
                                <th >
                                    <img class="post-avatar" width="70px" height="70px" src="{{asset('storage/'.$item->thumbnail)}}">
                                </th>
                                <td>{{$item->title}}</td>
                                <td>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content modal-create">
                <div class="modal-header">
                    <h5 class="modal-title" id="formModal">{{$this->editMode ? 'ویرایش  پست' : 'ساخت  پست'}}</h5>
                ...
                <div class="modal-body">
                    <form>
                        ...
                        <div class="form-group">
                            <label for="slug">خلاصه پست</label>
                            <textarea id="description" wire:model.lazy="description" rows="3"  placeholder="خلاصه  پست را وارد کنید"></textarea>
                            @error('description')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            <label for="content">توضیحات  پست</label>
                            <div wire:ignore>
                                <div id="content"></div>
                            </div>
                            @error('content')
                        ...
                        <div class="form-group">
                            <label for="slug">آدرس  ویدئو</label>
                            <input type="text" id="video_url" wire:model.lazy="video_url"
                                   class="form-control "
                                   placeholder="آدرس ویدئو پست را وارد کنید">
                            @error('video_url')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div  class="form-group">
                            <label>تگ ها</label>
                            <div wire:ignore class="input-field ">
                                <select onchange="@this.set('tag',this.value)" >
                                    <option value="" disabled selected>گزینه خود را انتخاب کنید</option>
                            ...
                            <div class="d-flex gap-1">
                                @foreach($tags_labels ?? [] as $tagitem_key => $tagitem)
                                    <button type="button" class="btn-hover btn-sm btn-border-radius color-7">{{$tagitem}} <i class="fa fa-times" wire:click="remove_item({{$tagitem_key}})"></i></button>
                                @endforeach
                            </div>
